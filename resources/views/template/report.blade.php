...
@section('title')
    @if(Input::has('submit') && hasPermission('reports.create'))
        <?php
        // $report_save_url = route('reports.create');
        $report_save_url = ''; // Todo : Temporary dummy. Need to
        //
        $report_save_url .= "?name=" . Input::get('report_name');
        ...
        ?>
        @if(Input::has('type') && Input::get('type')=='Module Generic Report')
            <?php
        ...
        @endif
    @endif
    @if (Input::has('report_name'))
        <h4>{{Input::get('report_name')}}</h4>
    @else
