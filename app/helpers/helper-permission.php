...
function hasAccess($permission, $user_id = false) {
    ...
    if (isset($user)) {
        ...
        if ((isset($permissions['superuser']) && $permissions['superuser'] == 1)) { // allow for super user
            $allowed = true;
        } else if (isset($permissions[$permission]) && $permissions[$permission] == 1) { // allow based on specific permission
            $allowed = true;
        }
        /*
        if ($user->hasAccess($permission)) {
            ...
            $allowed = false;
            // for the purpose of diagnosis store permission log in session.
...
function storePermissionsInSession() {
    if (user()) Session::put('permissions', user()->getMergedPermissions());
}